...
import { ScrollView, Text, View, Modal, ActivityIndicator, Alert } from 'react-native';
...
export default function ConfirmPlaceOrderScreen() {
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      const paypalOrder = await createPaypalOrder(booking.totalPrice, booking.id);
    ...
    <View className="flex-1 relative">
      ...
      >
        <View style={{ marginHorizontal: 12 }}>
          {propertyData && <BookingSummary booking={booking} property={{ ...propertyData, agent: propertyData.agent }} />}
        </View>
      </ScrollView>
